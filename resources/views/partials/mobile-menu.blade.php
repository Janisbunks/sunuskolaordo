              ...
              @foreach ($navigation as $item)
                  @if ($item->children)
                      <div x-data="{ open: false }" x-cloak x-on:click="open = !open" class="relative !ml-0 mt-0 border-t border-grey" @click.stop>
                          ...
                          <div x-show="open" :class="{ 'z-20 -mb-px' : open }" class="relative bg-lightDark" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-2" display="none">
                              @foreach ($item->children as $child)
                                  ...
                                  @else
                                      <div x-data="{ open: true }" class="relative !ml-0 mt-0 border-t border-grey pl-4" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform -translate-y-2" display>
