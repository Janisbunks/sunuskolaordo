  ...
  <ul class="siblings-list">
    @foreach ($siblings as $sibling)
      <li class="space-y-4 list-none">
        <a
          title="Lasīt vairāk @title"
          aria-label="Lasīt vairāk @title"
          href="{{ get_permalink($sibling->ID) }}"
          class="group sibling-link flex items-center gap-4 justify-between hover:text-cyan-500"
        >
          {{ $sibling->post_title }}<x-icon-arrow-right
            class="relative group-hover:-translate-x-2 flex-shrink-0 group-hover:scale-[2.0]"
          />
        </a>
      </li>
    @endforeach
