  ...
  <section class="py-8">
    <div class="container">
      ...
      @foreach($terms as $term)
        ...
        <a href="{{ $link }}" class="group relative block h-[300px] lg:h-[500px] overflow-hidden rounded-3xl shadow-2xl">
          ...
          <div class="absolute inset-0 flex flex-col justify-end p-10">
            <div class="transform transition-all duration-500 translate-y-4 group-hover:translate-y-0">
              <span class="inline-block px-4 py-1.5 mb-4 text-xs font-bold tracking-widest text-black uppercase bg-white/20 backdrop-blur-md rounded-full border border-white/30">
                {{ $term->count }} {{ _n('Produkts', 'Produkti', $term->count, 'radicle') }}
              </span>

              <h3 class="text-4xl font-extrabold text-black mb-4 tracking-tight">
                {{ $term->name }}
              </h3>

              <div class="flex items-center text-indigo-400 font-bold overflow-hidden">
                <span class="h-[2px] w-0 bg-indigo-500 transition-all duration-500 group-hover:w-12 mr-0 group-hover:mr-4"></span>
                <span class="opacity-0 -translate-x-10 transition-all duration-500 group-hover:opacity-100 group-hover:translate-x-0">
                  Apskatīt
                </span>
              </div>
            </div>
